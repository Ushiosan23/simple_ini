...
public final class NumberConverter {
	 ...
	 * @return a valid number object or {@link Optional#empty()} if the content is not valid
	 */
	 ...
	 *
	 * @param content the content to convert
	 * @return a valid number object or {@code null} if the content is not valid
	 */
	...
	private static <T extends Number> @NotNull Optional<T> toCustomNumberImpl(
		...
		@NotNull Apply.Result<Number, T> apply
	) {
		// Temporal result
		Number result = toNumberImpl(content);
		T conversion = result == null ? null:apply.apply(result);
		// Apply invocation
